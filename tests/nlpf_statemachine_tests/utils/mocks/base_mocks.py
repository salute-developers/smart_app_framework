...
class BaseMocks(MessageMocks):
    ...
    def user(
            self,
            message: SmartAppFromMessage | BaseMessage | dict | None = None,
            headers: dict | None = None,
            db_data: str = "{}",
            user_cls: type[SMUser] | None = SMUser,
    ) -> SMUser:
        """
        ## Мок объекта User.
    ...
    @staticmethod
    def text_preprocessing_result(payload: dict | None = None) -> TextPreprocessingResult:
